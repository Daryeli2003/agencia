...
if (isset($_GET['id'])) {
    ...
    $modelo = $modeloModel->leerUno($id_modelo);
}
?>
<!DOCTYPE html>
<html lang="en">
                ...
                ?>
                    <form action="<?php echo BASE_URL; ?>controllers/modeloController.php" method="POST" class="form-inline">
                        ...
                        <input type="text" id="nombre_modelo" name="nombre_modelo" 
                               value="<?php echo htmlspecialchars($modelo['Nombre_modelo']); ?>" 
                               required>
                        
                        <button type="submit" class="btn-principal">Actualizar Modelo</button>
                        <a href="<?php echo BASE_URL; ?>index.php?page=gestion_modelo" class="btn-secundario">Cancelar</a>
                    </form>
                <?php else: ?>
                    <p class="no-data">No se pudo cargar la información del modelo para editar.</p>
                <?php endif; ?>
            </div>
        </div>
