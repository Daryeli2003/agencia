...
$id = $_GET['id'] ?? null;
if (!$id) {
    header("Location: " . BASE_URL . "index.php?page=serviciosrealizados&error=id_invalido");
    exit;
}
$model = new MantenimientoModel();
$registro = $model->obtenerMantenimientoPorId($id);
if (!$registro) {
    header("Location: " . BASE_URL . "index.php?page=serviciosrealizados&error=no_encontrado");
    exit;
}
?>
...
<body>
    ...
    <div class="container">
            <h1>Editar Mantenimiento #<?= htmlspecialchars($id) ?></h1>
            ...
            <form action="<?php echo BASE_URL; ?>controllers/MantenimientoController.php" method="post">
                <input type="hidden" name="accion" value="editar">
                <input type="hidden" name="id_Mantenimiento" value="<?= htmlspecialchars($registro['id_Mantenimiento']) ?>">
                
                <div class="form-group">
                    <label for="Fecha">Fecha</label>
                    <input type="date" name="Fecha" value="<?= htmlspecialchars($registro['Fecha']) ?>" required>
                </div>
                <div class="form-group">
                    <label for="Descripcion">Descripción</label>
                    <textarea name="Descripcion" required><?= htmlspecialchars($registro['Descripcion']) ?></textarea>
                </div>
                <div class="form-group">
                    <label for="quien_autoriza">Quién autoriza</label>
                    <input type="text" name="quien_autoriza" value="<?= htmlspecialchars($registro['quien_autoriza']) ?>" required>
                </div>
                
                <div class="form-group">
                    <button type="submit" class="btn btn-primary">Actualizar</button>
                    
                    <a href="<?php echo BASE_URL; ?>index.php?page=serviciosrealizados" class="btn btn-danger">Cancelar</a>
                </div>
            </form>
        </div>
    </div>
